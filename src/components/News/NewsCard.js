...
import { faCalendarDays, faClock, faMapMarkerAlt, faUser, faTag } from '@fortawesome/free-solid-svg-icons';
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
            type === 'event' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
          }`}>
            {item.category}
          </span>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-4 mb-4 text-sm text-gray-600">
          ...
          {type === 'event' && item.time && (
            <div className="flex items-center gap-2">
              <FontAwesomeIcon icon={faClock} className="w-4 h-4 text-green-500" />
              <span>{item.time}</span>
            </div>
          )}
        ...
        {type === 'event' && item.organizer && (
          <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
            <FontAwesomeIcon icon={faTag} className="w-4 h-4 text-orange-500" />
            <span className="font-medium">Penyelenggara: {item.organizer}</span>
          </div>
        )}
