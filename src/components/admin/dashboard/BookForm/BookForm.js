...
const BookForm = ({ book, onSubmit, onCancel, isLoading = false }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.pages.trim()) newErrors.pages = 'Jumlah halaman wajib diisi';
    if (!formData.category) newErrors.category = 'Kategori wajib dipilih';
    if (!formData.synopsis.trim()) newErrors.synopsis = 'Sinopsis wajib diisi';
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {previewImage && (
            <div className="mt-2">
              <p className="text-sm font-medium text-gray-700 mb-2">Preview Cover:</p>
