...
export default function AdminLayananPage() {
    ...
    useEffect(() => {
        ...
        if (filterPriceRange !== 'all') {
            switch (filterPriceRange) {
                case 'low':
                    filtered = filtered.filter(
                        (pkg) => pkg.priceNumber < 500000
                    );
                    break;
                case 'medium':
                    filtered = filtered.filter(
                        (pkg) =>
                            pkg.priceNumber >= 500000 &&
                            pkg.priceNumber < 900000
                    );
                    break;
                case 'high':
                    filtered = filtered.filter(
                        (pkg) => pkg.priceNumber >= 900000
                    );
                    break;
            }
        }
    ...
    const handleToggleStatus = async (pkg) => {
        try {
            ...
            showNotification(
                'success',
                pkg.isActive ? 'Paket dinonaktifkan' : 'Paket diaktifkan'
            );
            await fetchPackages();
        } catch (error) {
            console.error(error);
                ...
                >
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-center">
                            <div
                                className={`card-hover bg-white rounded-2xl shadow-lg border-2 border-gray-100 overflow-hidden relative max-w-sm`}
                            >
                                <div
                                    className={`${selectedPackage.bgColor} ${selectedPackage.textColor} p-6 text-center`}
                                >
                                    ...
                                    <ul className="space-y-3 text-sm">
                                        {selectedPackage.features.map(
                                            (feature, index) => (
                                                <li
                                                    key={index}
                                                    className={`feature-item ${feature.highlight ? 'text-red-600 font-semibold' : ''}`}
                                                >
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p className="text-gray-900">
                                    Rp{' '}
                                    {selectedPackage.priceNumber?.toLocaleString(
                                        'id-ID'
                                    )}
                                </p>
                            </div>
                            <div>
                                ...
                                <p
                                    className={`${selectedPackage.isActive ? 'text-green-600' : 'text-red-600'} font-medium`}
                                >
