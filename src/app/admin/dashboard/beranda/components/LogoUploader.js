...
export default function LogoUploader({ logoUrl, onSuccess, setNotification }) {
  ...
  const handleUpload = async () => {
    try {
      ...
      setNotification({ type: 'success', message: 'Logo berhasil diunggah!' });
      onSuccess();
    } catch {
      setNotification({ type: 'error', message: 'Gagal mengunggah logo.' });
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      setNotification({ type: 'error', message: 'Gagal menghapus logo.' });
    }
  };
  ...
  return (
    <div className="p-4 border rounded space-y-4">
      ...
      <div className="space-x-2">
        ...
        {logoUrl && <Button variant="danger" onClick={() => setConfirmOpen(true)}>Hapus Logo</Button>}
      </div>
      {confirmOpen && (
        <ConfirmDeleteModal
