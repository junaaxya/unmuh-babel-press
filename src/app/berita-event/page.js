...
import { faNewspaper, faCalendarCheck, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
export default function BeritaEventPage() {
  ...
  const getFilterStats = () => {
    ...
    const eventCount = allData.filter(item => item.type === 'event').length;
    const upcomingCount = allData.filter(item => 
      item.type === 'event' && 
      (item.status === 'Upcoming' || new Date(item.date) > new Date())
    ).length;
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl md:text-2xl text-blue-100 mb-8">
              Tetap update dengan berita terbaru dan acara menarik dari Unmuh Babel Press
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                <div className="flex items-center justify-center mb-4">
                  <FontAwesomeIcon icon={faNewspaper} className="w-8 h-8 text-blue-200" />
                </div>
                <div className="text-3xl font-bold mb-2">{stats.newsCount}</div>
                <div className="text-blue-200">Artikel Berita</div>
              </div>
              
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                <div className="flex items-center justify-center mb-4">
                  <FontAwesomeIcon icon={faCalendarCheck} className="w-8 h-8 text-purple-200" />
                </div>
                <div className="text-3xl font-bold mb-2">{stats.eventCount}</div>
                <div className="text-purple-200">Total Event</div>
              </div>
              
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                <div className="flex items-center justify-center mb-4">
                  <FontAwesomeIcon icon={faCalendarCheck} className="w-8 h-8 text-green-200" />
                </div>
                <div className="text-3xl font-bold mb-2">{stats.upcomingCount}</div>
                <div className="text-green-200">Event Mendatang</div>
              </div>
            </div>
          </div>
        </div>
